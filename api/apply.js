// api/apply.js
// Vercel Serverless Function: accepts multipart/form-data (incl. file) and emails via SendGrid SMTP
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!file?.buffer) return res.status(400).json({ error: "Resume file is required" });

    // Create SMTP transport (SendGrid)
...
function parseMultipart(req) {
  return new Promise((resolve, reject) => {
    ...
    const fields = {};
    let file = { buffer: null, filename: "", mimetype: "" };
